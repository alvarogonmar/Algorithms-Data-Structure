...
public class ZeroMatrixSolution { // O(n*m)
  ...
  private void processRows(int[][] matrix) { // funcion para procesar las filas
    for (int row = 0; row < matrix.length; row++) { // recorremos las filas
      if (matrix[row][0] == 0) { // si la primera columna de la fila es 0
        setRowToZero(matrix, row); // establecemos la fila a 0
      }
    }
  }

  private void setRowToZero(int[][] matrix, int row) { // funcion para establecer la fila a 0
    for (int col = 0; col < matrix[0].length; col++) { // recorremos las columnas
      matrix[row][col] = 0; // establecemos la fila a 0
    }
  }

  private void processColumns(int[][] matrix) { // funcion para procesar las columnas
    for (int col = 0; col < matrix[0].length; col++) { // recorremos las columnas
      if (matrix[0][col] == 0) { // si la primera fila de la columna es 0
        setColToZero(matrix, col); // establecemos la columna a 0
      }
    }
  }
  ...
  private void setColToZero(int[][] matrix, int col) { // funcion para establecer la columna a 0
    for (int row = 0; row < matrix.length; row++) { // recorremos las filas
      matrix[row][col] = 0; // establecemos la columna a 0
    }
  }
}
